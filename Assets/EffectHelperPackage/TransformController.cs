...
{   
            ...
            foreach(Vec3ParaSet Vec3set in socket.Vec3ParaList){
                switch(Vec3set.mode){
                    case(TrasnsMode.POSITION):
                        socket.obj.transform.position = Vector3.Lerp(Vec3set.vec01,Vec3set.vec02,clamped_percent);
                        break;
                    case(TrasnsMode.LOCALPOSTTION):
                        socket.obj.transform.localPosition = Vector3.Lerp(Vec3set.vec01,Vec3set.vec02,clamped_percent);
                        break;
                    case(TrasnsMode.ROTATION):
                        socket.obj.transform.rotation = Quaternion.Euler(Vector3.Lerp(Vec3set.vec01,Vec3set.vec02,clamped_percent));
                        //renew default
                        defaultRotation = Quaternion.Euler(Vector3.Lerp(Vec3set.vec01,Vec3set.vec02,clamped_percent));
                        break;
                     case(TrasnsMode.LOCALROTATION):
    ...
    public IEnumerator SetVec3Fade(GameObject obj,TrasnsMode mode, Vector3 target, float duration){
        ...
        switch(mode){
            case(TrasnsMode.POSITION):
                Vector3 initPost = obj.transform.position;   
                while(timer <= duration){
                    obj.transform.position = Vector3.Lerp(initPost,target,timer/duration);
            ...
            break;
             case(TrasnsMode.LOCALPOSTTION):
                Vector3 initLocalPost = obj.transform.position;   
                while(timer <= duration){
                    obj.transform.localPosition = Vector3.Lerp(initLocalPost,target,timer/duration);
                    timer+=Time.deltaTime;
                    yield return null;
                }
                obj.transform.position = target ;
            ...
            case(TrasnsMode.ROTATION):
                ...
                while(timer <= duration){
                    obj.transform.rotation = Quaternion.Lerp(initRot,Quaternion.Euler(target),timer/duration) ;
                    timer+=Time.deltaTime;
                    yield return null;
                }
            ...
            break;
             case(TrasnsMode.LOCALROTATION):
                quaternion initLocalRot = obj.transform.rotation;   
                while(timer <= duration){
                    obj.transform.localRotation = Quaternion.Lerp(initLocalRot,Quaternion.Euler(target),timer/duration) ;
                    timer+=Time.deltaTime;
                    yield return null;
                }
                obj.transform.rotation = Quaternion.Euler(target);
